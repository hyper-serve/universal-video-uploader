...
| `styles` | `FileListStyles` | none | Slot map applied to internal elements (see [Slots](#slots) below) |
...
Pass a `styles` slot map to theme internal elements. Slots merge over defaults; local `style`/`emptyStyle` props win over slots. See [Theming](/core-concepts/theming/) for the canonical merge-order explanation.
