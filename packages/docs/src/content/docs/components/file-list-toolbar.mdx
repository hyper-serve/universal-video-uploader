...
The React Native `FileListToolbar` exposes the same slots plus two extras for the inner `<Text>` of each button (web doesn't need them — it uses inline icon children):
