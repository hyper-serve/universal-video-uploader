...
- A [Hyperserve](https://hyperserve.io/?utm_source=docs&utm_medium=body&utm_campaign=video-uploader&utm_content=quick-start) account (Free or Pay Per Use) and API key (for the example below)

## Minimal example

<Aside type="note">
  This example uses the [Hyperserve](https://hyperserve.io/?utm_source=docs&utm_medium=body&utm_campaign=video-uploader&utm_content=quick-start) adapter, the recommended starting point. If you're uploading to a different backend, skip to [Custom Adapter](/adapters/custom-adapter/).
</Aside>
...
The three callbacks (`createUpload`, `completeUpload`, and `pollVideoStatus`) must run server-side to keep your [Hyperserve](https://hyperserve.io/?utm_source=docs&utm_medium=body&utm_campaign=video-uploader&utm_content=quick-start) API key out of the browser. Each route must implement the following contract:

| Route | Receives | Returns |
|---|---|---|
| `POST /api/create-upload` | `{ name: string, ...uploadOptions }` | `{ videoId: string }` |
