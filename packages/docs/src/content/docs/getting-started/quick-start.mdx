...
- A [Hyperserve](https://hyperserve.io) account and API key (for the example below)
...
The three callbacks (`createUpload`, `completeUpload`, and `getVideoStatus`) must run server-side to keep your [Hyperserve](https://hyperserve.io) API key out of the browser. Each route must implement the following contract:
